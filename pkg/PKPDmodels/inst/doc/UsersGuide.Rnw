...
\maketitle
\begin{abstract}
  The \pkg{PKPDmodels} package provides a function, \code{PKPDmodel},
...
and other covariates such as dose.  The \code{LinPKmodel} function
...
$\Cl$,
<<bolus1cptSdVClExpr>>=
PK1expr("bolus", "sd", list(k ~ Cl/V))
@ 
or, to express the model in terms of $\log(V)$ and $\log(\Cl)$,
<<bolus1cptSdlVlClExpr>>=
PK1expr("bolus", "sd", list(k ~ Cl/V, Cl ~ exp(lCl), V ~ exp(lV)))
@
...
<<bolus_1cpt_sd_Vk_expr>>=
(bolus1cptSdVk <- PK1cmpt("bolus", "sd"))
@ 
...
<<oral1cptSdlkalVlCl>>=
(oral1cptSdlkalVlCl <-
 PK1cmpt("oral", "sd", list(ka ~ exp(lka), k ~ exp(lCl)/V, V ~ exp(lV))))
@ 
corresponding to the formula
<<oral1cptSdlkalVlCl>>=
PK1expr("oral", "sd", list(ka ~ exp(lka), k ~ exp(lCl)/V, V ~ exp(lV)))
@ 

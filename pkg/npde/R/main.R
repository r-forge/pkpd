...
#' verbose = FALSE, calc.npde=TRUE, calc.pd=TRUE,
#' method = "cholesky", cens.method = "cdf", units = list(x="",y=""), detect=TRUE)
#' @usage npde()
...
#' @param boolsave a boolean (T if graphs and results are to be saved to a
#' file, F otherwise), defaults to T	
...
#' to postscript ("eps")
#' @param verbose a boolean (T if messages are to be printed as each subject is
#' processed, F otherwise), defaults to FALSE
#' @param calc.npde a boolean (T if npde are to be computed, F otherwise),
#' defaults to TRUE
#' @param calc.pd a boolean (T if pd are to be computed, F otherwise), defaults
#' to TRUE
#' @param cens.method a character string indicating the method used to handle 
#' censored data (see \code{\link{npde.cens.method}})
#' defaults to cdf
#' @param method a character string indicating the method used to decorrelate
#' observed and simulated data in the computation of npde (see \code{\link{npde.decorrel.method}})
...
#' the dataset is on, defaults to TRUE
...
npde<-function() {
    ...
    opt<-list(boolsave=xinput$boolsave,namsav=xinput$namfile, type.graph=xinput$type.graph,verbose=xinput$verbose,calc.npde=xinput$calc.npde, calc.pd=xinput$calc.pd,method=xinput$method,cens.method=xinput$cens.method)
    npde.obj<-new(Class="NpdeObject",data=xdat,sim.data=xsim,options=opt)
    npde.obj["prefs"]<-set.npde.plotoptions(npde.obj)
...
#' @export
autonpde<-function(namobs,namsim,iid,ix,iy,imdv=0,icens=0,icov=0, iipred=0,boolsave=TRUE,namsav="output",type.graph="eps",verbose=FALSE, calc.npde=TRUE,calc.pd=TRUE,method="cholesky",cens.method="cdf", units=list(x="",y=""), detect=TRUE) {
...
#    if(cens.method=="cdf" & !calc.pd) {
    ...
    opt<-list(boolsave=boolsave,namsav=namsav,type.graph=type.graph, verbose=verbose,calc.npde=calc.npde, calc.pd=calc.pd,method=method, cens.method=cens.method)
    npde.obj<-new(Class="NpdeObject",data=xdat,sim.data=xsim,options=opt)
...
pdemenu<-function() {
    ...
    type.graph<-"eps";namegr<-nameres<-"";method<-"cholesky";cens.method<-"cdf"
    if(boolsave) {
    ...
    cok<-as.integer(readline(prompt="Which method should be used for the decorrelation (1-2) ? "))
    if(is.na(cok)) {
      cok<-1
      cat("       Cholesky method selected\n")
    }
    if(cok==2) method<-"inverse"
    ...
    cat("         4. ypred: an observation below the LOQ will be imputed to the population model prediction\n")
    cat("         5. ipred: an observation below the LOQ will be imputed to the individual model prediction\n")
    cok<-as.integer(readline(prompt="Which method should be used (1-5) ? "))
    if(is.na(cok)) {
      cok<-2
    ...
    cok<-readline(prompt="Automatic recognition of columns in the dataset (y/Y) [default=yes] ? ")
    if(tolower(cok)=="n"|tolower(cok)=="no") detect<-FALSE else detect<-TRUE
    return(list(namobs=namobs,namsim=namsim,iid=iid,ix=ix,iy=iy,imdv=imdv,icens=icens, iipred=iipred,icov=icov,boolsave=boolsave,type.graph=type.graph,namfile=namfile, calc.pd=calc.pd,calc.npde=calc.npde,verbose=verbose,method=method, cens.method=cens.method,detect=detect))
}

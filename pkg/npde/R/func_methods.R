...
computepd<-function(npdeObject,...) {
    ...
    object<-computepd.omit(npdeObject) else {
		  ...
		  if(npdeObject["options"]$cens.method %in% c("ipred","ypred"))
...
# Compute ypred
		ypred<-rowMeans(matsim)
		ypredall<-c(ypredall,ypred)
		if(npdeObject["options"]$cens.method=="ypred") ysuj[icens[idobs==isuj]==1]<-ypred[icens[idobs==isuj]==1]
# compute pd_ij
		tcomp<-apply(matsim,2,"<",ysuj)
		if(!is.matrix(tcomp)) tcomp<-t(as.matrix(tcomp))
		ycal<-rowMeans(tcomp)
	...
	pdfull[keep]<-pd
	ypredfull[keep]<-ypredall
	if(npdeObject["options"]$cens.method=="ypred") 
...
computenpde.omit<-function(npdeObject) {
	...
	for(isuj in unique(id)) {
		...
		x<-calcnpde(isuj,yobs,matsim,nrep,npdeObject["options"]$method, npdeObject["options"]$verbose)
		if(x$xerr>0) {
			xerr<-x$xerr
			break
		}
	...
	if(xerr>0) {
		...
		if(xerr==1) {
			if(npdeObject["options"]$method=="cholesky") cat("the Cholesky decomposition of the covariance matrix of the simulated data could not be obtained.\n")
			if(npdeObject["options"]$method=="inverse") cat("the covariance matrix of the simulated data could not be diagonalised through eigen().\n")
			if(npdeObject["options"]$method=="polar") cat("the Cholesky decomposition of the covariance matrix of the simulated data could not be obtained.\n")
		}
...
computenpde.loq<-function(npdeObject) {
	...
	ycens<-tab[,npdeObject["data"]["name.cens"]]
	ypred<-pde<-rep(NA,dim(tab)[1])
	if(npdeObject["options"]$cens.method%in% c("ipred","ypred","cdf")) 		
...
# else add U(0,1/nrep)
		...
		if(calc.npde) {
			y<-calcnpde.sim(ysuj,matsim,nrep,npdeObject["options"]$method)
			if(y$xerr==0) {
				pde<-y$pde
				if(npdeObject["options"]$ties)
					pde<-pde+1/(2*nrep)

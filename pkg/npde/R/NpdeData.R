...
##'   \item{set.npde.plotoptions(npde.data):}{Sets options for graphs of npde.data (internal method used in plots)}
##' }
...
#' 
##' @aliases set.npde.plotoptions set.npde.plotoptions,NpdeData-method
...
#' @seealso \code{\link{npde}}, \code{\link{autonpde}}
#' @keywords plot 
##' @S3method set.npde.plotoptions NpdeData
##' @export

set.npde.plotoptions.NpdeData<-function(object, ...) {
...
# Options for plot types
    ...
    cens.pch=8,
    obs.col="black",		# color to plot observed/censored data
    cens.col="red",
    lcol="black",
    ablinecol="black",
    ablinelty=2,
...
#' 
#' @usage replace.npde.plotoptions(plot.opt,...)
...
replace.npde.plotoptions<-function(plot.opt,...) {
...
# These arguments are used by other functions and may be passed on via "...", so we want to ignore them. Other arguments not in list will raise warnings
	...
	if(length(args1)>2) {
		...
		if(!is.na(i1)) {
			...
			if(is.na(match("obs.col",names(args1)))) plot.opt$obs.col<-eval(args1[[i1]])
		}
		i1<-match("pch",names(args1))
		if(!is.na(i1)) {
			plot.opt$pch<-eval(args1[[i1]])
			if(is.na(match("obs.pch",names(args1)))) plot.opt$obs.pch<-eval(args1[[i1]])
		}
		# Other arguments
...
# Plot the data, either as points or as lines grouped by x@name.group
    ...
    plot.opt<-set.npde.plotoptions(x)
    plot.opt$new<-TRUE
    ...
    if(has.cens && max(tab[,x@name.cens])==0) has.cens<-FALSE
      if(plot.type=="p" | plot.type=="b") {
      	if(has.cens) {
        plot(tab[,x@name.predictor],tab[,x@name.response],xlab=plot.opt$xlab, ylab=plot.opt$ylab,col=plot.opt$col,log=logtyp,xlim=plot.opt$xlim, ylim=plot.opt$ylim,main=plot.opt$main,cex=plot.opt$cex,cex.axis=plot.opt$cex.axis, cex.lab=plot.opt$cex.lab,type="n")
        points(tab[tab[,x@name.cens]==0,x@name.predictor], tab[tab[,x@name.cens]==0,x@name.response],col=plot.opt$obs.col, pch=plot.opt$obs.pch,cex=plot.opt$cex)
        points(tab[tab[,x@name.cens]==1,x@name.predictor], tab[tab[,x@name.cens]==1,x@name.response],col=plot.opt$cens.col, pch=plot.opt$cens.pch,cex=plot.opt$cex)
      	} else 
      		plot(tab[,x@name.predictor],tab[,x@name.response],xlab=plot.opt$xlab, ylab=plot.opt$ylab,col=plot.opt$col,pch=plot.opt$pch,log=logtyp,xlim=plot.opt$xlim, ylim=plot.opt$ylim,main=plot.opt$main,cex=plot.opt$cex,cex.axis=plot.opt$cex.axis, cex.lab=plot.opt$cex.lab)
      }
      if(plot.type=="l") {
        plot(tab[,x@name.predictor],tab[,x@name.response],xlab=plot.opt$xlab, ylab=plot.opt$ylab,col=plot.opt$col,lty=plot.opt$lty,lwd=plot.opt$lwd,type="n", log=logtyp,xlim=plot.opt$xlim,ylim=plot.opt$ylim,main=plot.opt$main, cex=plot.opt$cex,cex.axis=plot.opt$cex.axis, cex.lab=plot.opt$cex.lab)
      }
      if(plot.type=="l" | plot.type=="b") {
        for(isuj in unique(tab[,x@name.group])) {
          lines(tab[tab[,x@name.group]==isuj,x@name.predictor], tab[tab[,x@name.group]==isuj,x@name.response],col=plot.opt$lcol, lty=plot.opt$lty,lwd=plot.opt$lwd)
      }
      }
...
setMethod("read.npdeSimData","NpdeSimData",
    ...
    if(exists(object@name.simdata)) {
      ...
      dat<-try(read.table(object@name.simdata,header=header,sep=sep,na=na.strings))
      ...
      x1<-unlist(strsplit(as.character(dat[1,1]),";",fixed=TRUE))
      if(length(x1)>1) mysep<-";"
      if(mysep!="") dat<-read.table(object@name.simdata,na=c(".","NA"),sep=mysep)
      if(!is.numeric(dat[1,1]))
        dat<-read.table(object@name.simdata,na=c(".","NA"),header=TRUE,sep=mysep)
      if(!is.numeric(dat[1,1])) {
...
#' 
#' @usage npdeData(name.data,header=TRUE,sep="",na=c(".","NA"),name.group, name.predictor,name.response, name.covariates,name.cens,name.miss,name.ipred, units=list(x="",y="",covariates=c()),detect=TRUE,verbose=FALSE)
#' @param name.data name of the file containing the observed data, or a dataframe
#' containing the observed data
#' @param sep field separator (for files on disk)
#' @param na strings to be considered as indicating NA
#' @param header boolean indicating whether the file has a header (mandatory if 
#' detect is TRUE)
...
npdeData<-function(name.data,header=TRUE,sep="",na=c(".","NA"),name.group, name.predictor,name.response, name.covariates,name.cens,name.miss,name.ipred, units=list(x="",y="",covariates=c()),detect=TRUE,verbose=FALSE) {
...
#  showall(x)
  ...
  x1<-read.npdeData(x,header=header,sep=sep,na=na,detect=detect, verbose=verbose)
